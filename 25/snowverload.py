...
class UndirectedGraph(NamedTuple):
    ...
    def calculate_min_cut(self, expected_min_cut_size: int) -> tuple[set[str], set[str]]:
        ...
        graph. Rather than running the algorithm the traditional n^2 × ln(n) times (for a failure rate of 1 ÷ n), we'll
        just keep trying until we hit the expected min cut size.
